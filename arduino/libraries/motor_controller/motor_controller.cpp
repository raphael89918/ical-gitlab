...
void MotorController::motor_callback(const wheel_tokyo_weili::motor &msg)
{
    update_wheel(msg);
    control_wheel(msg);
//    pwm_log(msg);
...
void MotorController::update_wheel(const wheel_tokyo_weili::motor &msg)
{
    switch (m_driver_id)
    {
...
void MotorController::control_wheel(const wheel_tokyo_weili::motor &msg)
{
    switch (m_driver_id)
    {
    case TB6612_FNG:
...
void MotorController::pwm_log(const wheel_tokyo_weili::motor &msg)
{
    char fl_char[8];
    char fl_log[16];
...
void MotorController::stop()
{
    for(size_t i=0;i<m_driver_num;++i)
    {
        digitalWrite(m_motor_driver[i].pin[SLP], 0);
    }
}

bool MotorController::rosConnected()
{
    bool connected = m_nh.connected();
    return connected;
}
